...
def test_query_with_filters_returns_no_matches_with_no_error():
    # given - a set number of matches to request in the query and filters including non-existent document type
    num_matches = 3
    filters = {
    ...
    with TestClient(app) as client:
        # when - we call the endpoint with a non-zero number of matches and the defined filters
        response = client.post(f"/api/v1/filter-query/test/{num_matches}",
            json = filters,
            )
        # then - the response comes through successfully, but there are no matches
        assert response.status_code == 200
